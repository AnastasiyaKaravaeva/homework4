...
//Всех наемников в алфавитном порядке, у которых специализация не командир- убавить им зп на 1000 и вывести на консоль
        mercenaries.stream()
                .filter(mercenary -> mercenary.getSpecialization() != SpecializationEnum.COMANDOR)
                .sorted((m1, m2) -> m1.getName().compareTo(m2.getName()))
                .peek(mercenary -> mercenary.setSalary(mercenary.getSalary() - 1000))
...
//Вывести на экран пары Наемник-Человек, где ключом будет общее имя
        Map<String, Person> mercenaryPersonMap = mercenaries.stream()
                .collect(Collectors.toMap(Mercenary::getName, mercenary -> new Person(mercenary.getName(),(mercenary.getAge()))));
        System.out.println("");
